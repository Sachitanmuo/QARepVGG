...
def display_layer_pattern(layer):
    threebythree = layer.rbr_dense.state_dict()
    bn = layer.rbr_dense.bn.state_dict()
    onebyone = layer.rbr_1x1.state_dict()
    bn_total = layer.bn.state_dict()
    
    print(f"3x3 kernel shape: {threebythree['weight'].shape}")
    print(f"BatchNorm for 3x3 shape: {bn['weight'].shape}")
    print(f"1x1 kernel shape: {onebyone['weight'].shape}")
    print(f"Total BatchNorm shape: {bn_total['weight'].shape}")
